...
//        if (sids.isEmpty()) {
//            return websocketSet.stream().map(WebSocketServer::getSid).toList();
//        }
//        return websocketSet.stream().filter(webSocketServer -> sids.contains(webSocketServer.sid)).map(WebSocketServer::getSid).toList();
